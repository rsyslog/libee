...
int
callback(struct ee_event *event, char *name,int type,cJSON *item)
{
	char *valstr = NULL;
	es_str_t *val;
...
//printf("callback: string value %s\n", valstr);
	...
	val = es_newStrFromCStr(valstr, strlen(valstr));
	ee_addStrFieldToEvent(event, name, val);
...
void parse_and_callback(struct ee_event *event, cJSON *item, char *prefix)
{
	char *name;
	char *newprefix;
	int lenprefix;
...
//printf("parse_and_callback, item %p, item->string %p, prefix(%d): '%s'\n", item, item->string, strlen(prefix), prefix);
	while (item)
	{
		...
		dorecurse = callback(event, newprefix, item->type, item);
		if (item->child && dorecurse)
			parse_and_callback(event, item->child,newprefix);
		item=item->next;
		if(bNeedFree)
			free(newprefix);
	}
